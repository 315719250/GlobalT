...
@interface TipsWebVC ()<UIWebViewDelegate>
...
-(void)createBtn
{
    ...
    //设置按钮选中状态
    btn.selected = NO;
    NSArray *arr = [FMDBTool searchTraviling];
    for (TravelingTipsModel *m in arr) {
...
- (void)editAction:(UIButton *)btn
{
    ...
    if (btn.selected) {
        [FMDBTool addTravilingTips:self.model];
    }else
    {
        [FMDBTool deleteTravilingTips:self.model];
    }
