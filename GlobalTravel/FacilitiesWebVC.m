...
@interface FacilitiesWebVC ()<UIWebViewDelegate>
...
@implementation FacilitiesWebVC
...
-(void)createBtn
{
    ...
    //设置按钮选中状态
    btn.selected = NO;
    NSArray *arr = [FMDBTool searchTravelfacilities];
    for (FacilitiesModel *m in arr) {
...
- (void)editAction:(UIButton *)btn
{
    ...
    if (btn.selected) {
        [FMDBTool addTravelfacilities:self.model];
    }else
    {
        [FMDBTool deleteTravelfacilities:self.model];
    }
